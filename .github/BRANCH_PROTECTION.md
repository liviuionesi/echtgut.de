...
The repository starts private (see REQUIREMENTS.md NFR-1) and on GitHub
Free tier, which doesn't support branch protection rules.

## Options

### Option 1: Make Repository Public
```bash
gh repo edit liviuionesi/echtgut.de --visibility public
```
...
1. **Never commit directly to `main`**
   - Task work happens on `develop` (see `CLAUDE.md`); `main` only
     receives a `develop` → `main` merge once a Story genuinely meets
     Definition of Done
   - Opening a PR for that merge is optional while there's one
     collaborator — fine to open anyway as a readable changelog

2. **Always get code review**
   - An independent review pass (human or a second AI pass — see
     `CLAUDE.md`'s work loop) before a Story is considered closeable
...
https://github.com/liviuionesi/echtgut.de/settings/branches
