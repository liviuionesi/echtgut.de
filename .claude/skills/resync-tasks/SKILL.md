...
obvious (e.g. "NFR" → "non-functional requirement", "ADR" → "architecture
decision record").
...
discipline — both matter, accuracy first, clarity always. See CLAUDE.md's
"Issue hygiene (mandatory)" section, which this skill operationalizes.

## A concrete before/after (calibration reference, adapted from the
incident on lmdb.dev that created this skill)

**Before (too long, too dense, keeps growing):**
```
## Acceptance Criteria
- [ ] apply/destroy round-trip with zero manual console steps — not yet attempted
- [ ] Stays within free-tier budget — can't confirm without a live apply
- [ ] Ingestion job runs on the schedule — can't confirm without a live apply

## Still open before this can close
Same shape as the last issue's: infra code is written and self-consistent,
but nothing has actually run against a real deploy target. Needs, in
order: (1) provision the target by hand, (2) configure credentials, (3) a
real deploy → hit the endpoint → tear down round-trip, watching for the
memory-sizing risk noted above...
```
(and after a live run, this grew to five numbered paragraphs of findings,
each several sentences long.)

**After (tight, plain, rescoped where the original plan didn't hold):**
```
## Task
Run the ingestion scheduler against a real feed source instead of the
recorded fixture.

## Acceptance Criteria
- [x] Scheduled run fires, fetches, persists — verified against a real feed (2026-08-01)
- [x] Stays within free-tier budget — confirmed via provider dashboard
- [x] Duplicate entries are deduplicated on re-run

## Notes
- Retrying a failed fetch mid-run duplicates partial writes — no code fix
...
1. **Discover.** `gh issue list --repo liviuionesi/echtgut.de --state open --limit 200`.
...
2. **Dispatch one Agent per issue, in parallel.** Give each agent:
   ...
   - Instructions:
     1. `gh issue view <n> --repo liviuionesi/echtgut.de --json body,comments,state,title`.
     2. Ground the rewrite in real state — check relevant code/commits
        ...
        text. Do not re-run expensive live verification (a real ingestion
        run against a live feed, full test suites) just for this sweep —
        if the issue already documents a real prior verification, keep
        that evidence, just state it briefly.
     3. Rewrite the body: a short task description in plain bullets or a
        ...
        reality in one line instead of leaving it unchecked forever.
     ...
     6. If the issue is now genuinely, verifiably done against its own
        ...
        comment — per CLAUDE.md's issue-hygiene rules, never leave a done
        issue open, but never close on a technicality either.
...
- Not a re-verification pass — don't re-run live deploys or full test
